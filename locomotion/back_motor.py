...
RIGHT_MOTOR2_IN3 = 11 # Right Motor 2 Forward
RIGHT_MOTOR2_IN4 = 8  # Right Motor 2 Backward
...
left_pwm = GPIO.PWM(LEFT_PWM, FREQ)  # PWM on LEFT_PWM pin
left_pwm.start(0)  # Start with motors disabled

right_pwm = GPIO.PWM(RIGHT_PWM, FREQ)  # PWM on RIGHT_PWM pin
right_pwm.start(0)  # Start with motors disabled
...
# Function to test a single motor in forward or backward direction
def test_motor(pin1, pin2, motor_name, pwm_pin, direction):
    ...
    while True:
        ...
        if len(user_input) == 3:
            ...
            if motor == 'r' and motor_num == 1:
                # Right Motor 1
                test_motor(RIGHT_MOTOR1_IN3, RIGHT_MOTOR1_IN4, "Right Motor 1", right_pwm, direction)
            elif motor == 'r' and motor_num == 2:
                # Right Motor 2
                test_motor(RIGHT_MOTOR2_IN3, RIGHT_MOTOR2_IN4, "Right Motor 2", right_pwm, direction)
            elif motor == 'l' and motor_num == 1:
                # Left Motor 1
                test_motor(LEFT_MOTOR1_IN1, LEFT_MOTOR1_IN2, "Left Motor 1", left_pwm, direction)
            elif motor == 'l' and motor_num == 2:
                # Left Motor 2
                test_motor(LEFT_MOTOR2_IN1, LEFT_MOTOR2_IN2, "Left Motor 2", left_pwm, direction)
            else:
